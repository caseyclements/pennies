...
def test_present_value():
    ...
    assert np.isclose(pv_dispatch, expected_contract_pv)


def test_contract_present_value_with_rates_zero():
    interest = 0.00
    crv = ConstantDiscountRateCurve(dt_valuation=dt_val, zero_rate=interest)
    zero_rates_market = RatesTermStructure.of_single_curve(dt_val, crv)
    pv = present_value(bullet, zero_rates_market, ccy)
    assert np.allclose(pv, notional)
